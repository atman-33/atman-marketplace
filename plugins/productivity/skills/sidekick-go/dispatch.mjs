#!/usr/bin/env node
// sidekick-go dispatcher: launch a persistent helper agent in its own zellij
// pane, wired to message back into the CALLER's own (already-running) pane
// via the launch-team message bus. Unlike handoff-go (baton fully
// transferred, no return channel), the caller pane keeps running and can
// iterate with the same helper -- review -> fix -> re-review -- without the
// helper losing context each round.
//
// Reuses launch-team's pane/messaging primitives directly (team-lib.mjs,
// bus.mjs) rather than duplicating them: the roster/bus format is generic
// enough to represent a 2-party caller/helper pair.
//
// Usage:
...
// zellij exports the id of the pane a process is running in via
// ZELLIJ_PANE_ID -- unlike `is_focused` (which tracks live UI attention and
// can point at a pane the user merely clicked into), this is fixed to the
// pane that actually launched this script, so replies always route back here.
function findCallerPaneId() {
  const raw = process.env.ZELLIJ_PANE_ID;
  if (!raw) fail('ZELLIJ_PANE_ID is not set -- could not identify the current pane.');
...
function launchHelperPane({ session, cwd, sessionId, rolePromptPath, dryRun }) {
  const claudeArgs = [
  ...
  const zellijArgs = [
    '--session', session, 'action', 'new-pane',
    '--name', 'helper', '--cwd', cwd,
    '--', 'claude', ...claudeArgs,
  ];
  if (dryRun) {
    console.log(`[dry-run] zellij ${zellijArgs.map((a) => (/\s/.test(a) ? JSON.stringify(a) : a)).join(' ')}`);
    return 'dry-run-helper';
  }
  const r = spawnSync('zellij', zellijArgs, { encoding: 'utf8' });
  if ((r.status ?? 1) !== 0) {
    fail(`failed to launch helper pane: ${r.stderr?.trim() || r.error?.message || 'unknown error'}`);
  }
...
function main() {
  ...
  if (!session) {
    fail(
      'sidekick-go must run inside a zellij session (ZELLIJ_SESSION_NAME is not set).\n' +
      '  Start zellij first, then re-run the skill from inside it.',
    );
  }
  if (!zellijOnPath()) fail('zellij was not found on PATH.');
  ...
  const paneId = launchHelperPane({ session, cwd: args.cwd, sessionId, rolePromptPath, dryRun: args.dryRun });
  ...
  console.log(`\nsidekick-go: started helper (${args.role}) in zellij session "${session}".`);
