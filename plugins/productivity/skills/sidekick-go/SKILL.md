...
`C:\Users\<user>\.claude\plugins\cache\atman-marketplace\productivity\<version>\skills\sidekick-go\dispatch.mjs`

Add `--dry-run` to print the zellij commands without launching. There is no A/B/C fallback here (unlike `handoff-go`): pane-to-pane messaging requires zellij, so the script fails fast with instructions if no session is active.
