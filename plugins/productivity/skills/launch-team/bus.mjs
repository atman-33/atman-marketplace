...
// Claude Code prompt (via zellij write-chars + Enter), so the teammate acts on
// it immediately — no polling, no inbox files, no re-resume.
//
...
function cmdRoster(args) {
  ...
  console.log(`Team: ${roster.team}  (zellij session: ${roster.session})`);
