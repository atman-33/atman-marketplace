#!/usr/bin/env node
// launch-team launcher: read a team config, open one named zellij pane per
// agent running a persistent `claude` session, write the shared roster, and
// kick off the orchestrator with the task brief. Deterministic half of the
// skill — keeps zellij wiring and quoting out of the LLM layer.
//
// Usage:
...
function launchPane({ session, agent, cwd, rolePromptPath, dryRun }) {
  const claudeArgs = [
  ...
  if ((r.status ?? 1) !== 0) {
    fail(`failed to launch pane for "${agent.id}": ${r.stderr?.trim() || r.error?.message || 'unknown error'}`);
  }
  // zellij new-pane prints the new pane id; normalize a bare number to the
  // "terminal_N" shape list-panes reports so the roster hint matches later.
...
function main() {
  ...
  if (!session) {
    fail(
      'launch-team must run inside a zellij session (ZELLIJ_SESSION_NAME is not set).\n' +
  ...
  for (const agent of ordered) {
    const paneId = launchPane({ session, agent, cwd: args.cwd, rolePromptPath: agent.rolePromptPath, dryRun: args.dryRun });
  ...
  // Summary.
  console.log(`\nlaunch-team: started ${cfg.agents.length} agents in zellij session "${session}".`);
