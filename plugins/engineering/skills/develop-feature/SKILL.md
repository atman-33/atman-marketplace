...
2. **Implement with TDD.** Delegate to the **heavy-implementer** agent (via the
   `Agent` tool): give it the spec and instruct it to invoke the **tdd** skill
   itself, implementing test-first (red → green → refactor) and iterating
   (running the test suite, lint/build as applicable, delegating verbose test
   runs to `test-runner` if useful) until the feature is complete and all
   checks pass. Take only its report (files changed, key decisions,
   verification results) — do not re-read its diff or re-run its checks
   yourself.
...
4. **Self-verify.** Delegate to the **heavy-implementer** agent (via the
   `Agent` tool): instruct it to invoke the **verify** skill itself, run the
   app, and confirm the change behaves as specified before the user looks at
   it. If verify finds a problem, the agent should fix it and re-verify within
   its own run; only escalate back here if it cannot resolve the issue itself.
   Take only its report — do not re-run verify yourself.
...
- Steps 2 and 4 run inside a delegated `heavy-implementer` agent's own
  context. Rely on its report contract (files changed, key decisions,
  verification results) rather than re-deriving what it did — do not paste or
  re-read its full diff, test output, or logs into this conversation.
- If TDD implementation (step 2) surfaces a blocker (ambiguous spec, a check
  that cannot be made to pass), the delegated agent should report it rather
  than guessing; stop and surface it to the user.
- If self-verify (step 4) reports a problem it could not resolve itself, or
  the user (step 6) reports a problem, fix it (re-delegate as needed),
...
- Files changed, once implementation is done (from the delegated agent's
  report at steps 2 and 4).
