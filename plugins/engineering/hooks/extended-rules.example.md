---
paths:
  - ../atman-marketplace/plugins/engineering/hooks/**
...
`C:/repos/agent-harness/.claude/rules-ex/`). They are a complement to a target
...
- Each glob is resolved **relative to the workspace root (cwd)**, so use `..` to
  reach sibling repos: `../atman-marketplace/plugins/**/*.ts`.
